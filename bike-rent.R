...
#load dataset
df=read_csv("bike-rent-data.csv",skip_empty_rows = TRUE)
...
names(df)[4] = "year"
names(df)[5] = "month"
names(df)[9] = "weather_condition"
names(df)[10]= "temperature"
names(df)[11]= "felling_temperature"
names(df)[12] = "humidity"
names(df)[16] = "total_count"
...
#missing value analysis using mice package
library(mice)
md.pattern(df,rotate.names = TRUE)   ##detect missing value pattern with graph
### outlier analysis
...
#val = df_numeric[which(df_numeric %in% boxplot.stats(df_numeric)$out)]
